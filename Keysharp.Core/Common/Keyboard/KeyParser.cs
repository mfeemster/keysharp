		...
		internal static Keys ParseKey(string name)
		{
			...
			try
			{
				if (value == Keys.None)
				{
					...
					{
						case " ": value = Keys.Space; break;//Put space and newline at the top since they're the most common.//MATT

						case "\r": value = Keys.Enter; break;
						...
						//What about altgr? AHK takes significant pain to ensure it's treated correctly.//MATT
						...
						default:
							_ = Enum.TryParse(name, true, out value);
							break;
					}
				}
			}
						...
						if (!scan)
							...
							switch (buf[1].ToString().ToLowerInvariant())
							{
								case Core.Keyword_Raw:
									_ = buf[0].Append(sequence, i, sequence.Length - i);
									i = sequence.Length;
									break;
							}
