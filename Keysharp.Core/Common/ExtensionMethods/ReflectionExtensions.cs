...
namespace System.Reflection
{
	public static class ReflectionExtensions
	{
		public static object ArgumentAdjustedInvoke(this MethodInfo mi, object inst, params object[] obj)
		{
			...
			var isVariadic = parameters.Length > 0 && parameters[parameters.Length - 1].ParameterType == typeof(object[]);

			if (parameters.Length == 0)
				return mi.ControlCheckedInvoke(inst, null);

			if (!isVariadic && parameters.Length == obj.Length)
			...
			{
				if (i == newobj.Length - 1 && isVariadic)//Check if the last param is variadic, and if so, put all remaining params there.
				{
					var newi = i;
