...
namespace Keysharp.Core.Windows
{
	internal class WindowsHookThread : Keysharp.Core.Common.Threading.HookThread
	{
							  ...
							  bool keyUp, Keysharp.Core.Common.Threading.KeyHistoryItem keyHistoryCurr, uint hotkeyIDToPost)
		{
			...
			{
				var input_level = InputLevelFromInfo((uint)kbd.dwExtraInfo);
		...
		internal override void ChangeHookState(List<HotkeyDefinition> hks, HookType whichHook, HookType whichHookAlways)
		{
			...
			if (kvk == null)  // Since it's an initialized global, this indicates that all 4 objects are not yet allocated.
			{
				...
				kvk[VK_CAPITAL].forceToggle = Keyboard.toggleStates;
				kvk[VK_NUMLOCK].forceToggle = Keyboard.toggleStates;
			}
			...
			for (var i = 0; i < hks.Count; ++i)
			{
				...
				if (hkIsCustomCombo)
				{
					if (hk.modifierVK != 0)
					{
						...
						else
						{
							...
							if ((hk.noSuppress & HotkeyDefinition.NO_SUPPRESS_PREFIX) != 0)
								kvk[hk.modifierVK].noSuppress |= HotkeyDefinition.NO_SUPPRESS_PREFIX;
						}
					}
					else //if (hk.mModifierSC)
					{
						...
						else
						{
							...
							if ((hk.noSuppress & HotkeyDefinition.NO_SUPPRESS_PREFIX) != 0)
								ksc[hk.modifierSC].noSuppress |= HotkeyDefinition.NO_SUPPRESS_PREFIX;
				...
				// !SEND_NOSUPPRESS_PREFIX_KEY_ON_RELEASE // Search for this symbol for details.
				else
				{
				...
				hkSorted[hkSortedCount].idWithFlags = hk.hookAction != 0 ? hk.hookAction : hotkeyIdWithFlags;
				hkSorted[hkSortedCount].vk = hk.vk;
				hkSorted[hkSortedCount].sc = hk.sc;
				hkSorted[hkSortedCount].modifiers = hk.modifiers;
				hkSorted[hkSortedCount].modifiersLR = hk.modifiersLR;
				hkSorted[hkSortedCount].allowExtraModifiers = hk.allowExtraModifiers;
				++hkSortedCount;
			}
				...
				for (var i = 0; i < hkSortedCount; ++i)
				{
					...
					for (modifiersLR = 0; modifiersLR <= MODLR_MAX; ++modifiersLR)  // For each possible LR value.
					{
						...
						else
						{
							...
							if (thisHkId >= HotkeyDefinition.shk.Count || prevHkId >= HotkeyDefinition.shk.Count) // AltTab hotkey.
							...
							{
								var prevHk = HotkeyDefinition.shk[prevHkId];
							...
							else
							{
								...
								{
									var new_up_hk = HotkeyDefinition.shk[(int)newUpId & HotkeyDefinition.HOTKEY_ID_MASK];
		...
		// neutral one.
		{
			...
			for (var input = Keysharp.Scripting.Script.input; input != null; input = input.Prev)
			{
			...
			var activeWindowKeybdLayout = GetKeyboardLayout(Keysharp.Core.Common.Window.WindowManagerProvider.Instance.GetFocusedCtrlThread(ref tempzero, activeWindow));
			...
			if (uwpHwndChecked != activeWindow)
			{
				...
				_ = WindowsAPI.GetClassName(activeWindow, class_name, 32);
		...
		/// </summary>
		internal IntPtr LowLevelCommon(IntPtr hook, int code, long wParam, ref KBDLLHOOKSTRUCT kbd, ref MSDLLHOOKSTRUCT mouse, int vk, int sc, bool keyUp, uint eventFlags)
		{
			var hotkeyIdToPost = (uint)HotkeyDefinition.HOTKEY_ID_INVALID; // Set default.
			var isIgnored = KeyboardMouseSender.IsIgnored(kbd.dwExtraInfo);
			...
			else
			{
				...
				if (fore_win != IntPtr.Zero)
				{
					if (fore_win != Keysharp.Scripting.Script.historyHwndPrev)
					{
						...
						var wnd = Form.FromHandle(fore_win) is Control ctrl ? ctrl : Keysharp.Scripting.Script.mainWindow;
			...
			if (isIgnored)
			{
				...
				if (kbd.dwExtraInfo == KeyPhysIgnore && keyUp && prefixKey == thisKey)
				{
			...
			if (hook == mouseHook)
			{
				...
				if ((vk == VK_LBUTTON || vk == VK_RBUTTON)
						&& (Keysharp.Scripting.Script.menuIsVisible != MenuType.None // Ordered for short-circuit performance.
							|| ((menuHwnd = FindWindow("#32768", null)) != IntPtr.Zero
								&& GetWindowThreadProcessId(menuHwnd, out _) == Keysharp.Core.Processes.MainThreadID))) // Don't call GetCurrentThreadId() because our thread is different than main's.
				{
			...
			if (prefixKey != null && prefixKey != thisKey && !keyUp) // There is a prefix key being held down and the user has now pressed some other key.
				if ((hook == kbdHook ? (thisKey.asModifiersLR == 0 ? 1 : 0) : prefixKey.asModifiersLR) != 0)//Use ^ xor to toggle.
			...
			if (thisKey.usedAsPrefix != 0 && !keyUp && (prefixKey == null || !thisKey.usedAsSuffix || thisKey == prefixKey))
			{
				...
				if (!(hasNoEnabledSuffixes = (thisKey.usedAsPrefix == KeyType.PREFIX_ACTUAL)
											 && HotkeyDefinition.PrefixHasNoEnabledSuffixes(scTakesPrecedence ? sc : vk, scTakesPrecedence)))
				{
				...
				if (thisKey.usedAsSuffix) // v1.0.41: Added this check to avoid doing all of the below when unnecessary.
				{
					...
					if (modifiersLRnew != 0 || hasNoEnabledSuffixes || (thisKey.noSuppress & HotkeyDefinition.NO_SUPPRESS_PREFIX) != 0)
					{
						...
						else // hotkey_id_with_flags is either HotkeyDefinition.HOTKEY_ID_INVALID or a valid key-down hotkey.
						{
			...
			if (thisKey.usedAsPrefix != 0 && keyUp) // If these are true, hotkey_id_with_flags should be valid only by means of this_key.hotkey_to_fire_upon_release.
			{
				...
				if (thisToggleKeyCanBeToggled) // Always false if our caller is the mouse hook.
				{
					...
					if (thisKey.wasJustUsed != 0  // AS_PREFIX or AS_PREFIX_FOR_HOTKEY.
			...
			////////////////////////////////////////////////////////////////////////////////////////////////////
			var fireWithNoSuppress = false; // Set default.
			...
			if (hotkeyIdWithFlags == HotkeyDefinition.HOTKEY_ID_INVALID)  // Since above didn't find a hotkey, check if modifiers+this_key qualifies a firing.
			{
				...
				else if (hotkeyIdWithFlags != HotkeyDefinition.HOTKEY_ID_INVALID) // hotkey_id_with_flags is a valid key-down hotkey.
				{
					...
					if (keyUp)
					{
						...
						if (hotkeyIdTemp < HotkeyDefinition.shk.Count)
						{
							// Fixed for v1.1.33.01: Any key-up hotkey already found by the custom combo section
				...
				if (hotkeyIdWithFlags == HotkeyDefinition.HOTKEY_ID_INVALID)
				{
					...
					_ = HotkeyDefinition.CriterionFiringIsCertain(ref thisKey.hotkeyToFireUponRelease // firing_is_certain==false under these conditions, so no need to check it.
							...
							, (uint)kbd.dwExtraInfo // May affect the result due to #InputLevel.  Assume the key-up's SendLevel will be the same as the key-down.
			...
			if ((thisKey.noSuppress & HotkeyDefinition.NO_SUPPRESS_NEXT_UP_EVENT) != 0 && keyUp)
			{
				fireWithNoSuppress = true; // In spite of this being a key-up, there may be circumstances in which this was already true due to action above.
			...
			if (keyUp)
			{
				...
				if (fireWithNoSuppress) // Plus we know it's not a modifier since otherwise it would've returned above.
				{
					...
					/*  only enalbe this for:
					    SEND_NOSUPPRESS_PREFIX_KEY_ON_RELEASE
			...
			else // Key Down
			{
				...
				if (prefixKey != null && prefixKey.asModifiersLR != 0)
					prefixKey.wasJustUsed = KeyType.AS_PREFIX_FOR_HOTKEY;
				...
				else if (vk == VK_LMENU || vk == VK_RMENU)
				{
					...
					kbdMsSender.SendKeyEvent(KeyEventTypes.KeyUp, vk, sc, IntPtr.Zero, false, KeyboardMouseSender.KeyBlockThis);
				}
			}
		...
		internal IntPtr LowLevelMouseHandler(int code, IntPtr param, ref MSDLLHOOKSTRUCT lParam)
		{
			...
			if ((lParam.flags & LLMHF_INJECTED) == 0) // Physical mouse movement or button action (uses LLMHF vs. LLKHF).
				Keysharp.Scripting.Script.timeLastInputPhysical = Keysharp.Scripting.Script.timeLastInputMouse = DateTime.Now;
			...
			if (iwParam == WM_MOUSEMOVE) // Only after updating for physical input, above, is this checked.
				return (Keyboard.blockMouseMove && ((lParam.flags & LLMHF_INJECTED) == 0)) ? new IntPtr(1) : CallNextHookEx(mouseHook, code, param, ref lParam);
				...
				switch (vk)
				{
					...
					case VK_CONTROL:
						...
						if (!alwaysSetAsPrefix)
						{
							...
							{
								var h = HotkeyDefinition.shk[i];  // For performance and convenience.
		...
		// neutral one.
		{
			...
			if (hotkeyIDToPost != HotkeyDefinition.HOTKEY_ID_INVALID)
			{
				var input_level = KeyboardMouseSender.InputLevelFromInfo((uint)lParam.dwExtraInfo);
				_ = channel.Writer.TryWrite(new KeysharpMsg()
				{
					message = (uint)UserMessages.AHK_HOOK_HOTKEY,
					wParam = new IntPtr(hotkeyIDToPost),
		...
		// that it was in the middle of, though it could start something new immediately after).
		{
				...
				if (channel.Writer.TryWrite(new KeysharpMsg()
			{
				...
				{
					while (!hookSynced)
						Keysharp.Core.Flow.SleepWithoutInterruption();
				}
			}
		}
				...
				await foreach (var item in reader.ReadAllAsync())
				{
					...
					if (item is KeysharpMsg msg)
					{
						...
						switch (msg.message)//Almost none of this is going to work until we figure out how threads are going to work.
						{
							...
							default: // hotkey
								var hk_id = wParamVal & HotkeyDefinition.HOTKEY_ID_MASK;

								if (hk_id >= HotkeyDefinition.shk.Count) // Invalid hotkey ID.
									continue;

								hk = HotkeyDefinition.shk[(int)hk_id];
		...
		private IntPtr LowLevelKeybdHandler(int code, IntPtr wParam, ref KBDLLHOOKSTRUCT lParam)//Might be nice to see if this can just return an int.//TODO
		{
			...
			if (lParam.scanCode == SC_FAKE_LCTRL && altGrExtraInfo != 0 && (lParam.flags & LLKHF_INJECTED) != 0)
			{
				...
				lParam.dwExtraInfo = altGrExtraInfo;
			}
		...
		private bool SystemHasAnotherdHook(ref System.Threading.Mutex existingMutex, string name)
		{
			...
			var last_error = WindowsAPI.GetLastError();
	...
	{
		AHK_HOOK_HOTKEY = WindowsAPI.WM_USER, AHK_HOTSTRING, AHK_USER_MENU, AHK_DIALOG, AHK_NOTIFYICON
		...
		, AHK_GUI_ACTION = WindowsAPI.WM_USER + 20 // Avoid WM_USER+100/101 and vicinity.  See below comment.
