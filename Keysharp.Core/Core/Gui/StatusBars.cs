...
namespace Keysharp.Core
{
	...
	{
		private static StatusStrip DefaultStatusBar
		{
			get
			{
				var gui = GuiHelper.DefaultGui;
				return gui == null ? null : ((GuiInfo)gui.Tag).StatusBar;
			}
		}
		...
		public static void SB_SetIcon(string Filename, int IconNumber, int PartNumber)
		{
			...
			if ((PartNumber != 0 && PartNumber > status.Items.Count) || PartNumber < 0)
				return;

			try { status.Items[PartNumber].Image = new Icon(Filename).ToBitmap(); }//MATT
			catch (Exception) { }
		}
		...
		public static void SB_SetParts(object[] WidthN)
		{
			...
			{
				//status.ShowPanels = false;//MATT
				status.Items.Clear();
			}
			else
			{
				foreach (var width in WidthN)
					_ = status.Items.Add(new ToolStripStatusLabel { Width = width is int w ? w : 100 });
		...
		public static void SB_SetText(string NewText, int PartNumber, int Style)
		{
			...
			PartNumber--;
			var max = status.Items.Count;

			if (PartNumber == max)
			{
				_ = status.Items.Add(string.Empty);
				max++;
			}
			...
			if (status.Items[PartNumber] is ToolStripStatusLabel tssl)
			{
				tssl.Text = NewText;
				var border = tssl.BorderStyle;
				...
				switch (Style)
				{
					...
					case 1:
						border = Border3DStyle.Flat;//Was "none".//MATT
						break;
