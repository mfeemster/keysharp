namespace Keysharp.Core
{
	public static class ListViews
	{
		...
		//public static ListView DefaultListView
		//{
		//  get
		//  {
		...
		public static int LV_Add(string options, string[] fields = null)
		{
			...
			//_ = LV_Modify(item.Index + 1, options, fields);
			//return item.Index;
		}
		...
		public static int LV_GetCount(string type = null)
		{
			...
			//if (type == Core.Keyword_Column || type.Length == 1 && type[0] == Core.Keyword_Column[0] || type.Length == 3 && Core.Keyword_Column.StartsWith(type))
			//  return list.Columns.Count;
			//if (type == Core.Keyword_Selected || type.Length == 1 && type[0] == Core.Keyword_Selected[0])
			//{
			//  var selected = 0;
			//  foreach (ListViewItem item in list.Items)
			//      if (item.Selected)
		...
		public static int LV_GetNext(int index, string type = null)
		{
			...
			//  return 0;
			//type = string.IsNullOrEmpty(type) ? string.Empty : type.ToLowerInvariant();
			//for (var i = Math.Max(0, index); i < list.Items.Count; i++)
			//{
			...
			//      if (item.Selected)
			//          return n;
			//  if (type == Core.Keyword_Checked || type.Length == 1 && type[0] == Core.Keyword_Checked[0])
			//      if (item.Checked)
			//          return n;
			//  if (type == Core.Keyword_Focused || type.Length == 1 && type[0] == Core.Keyword_Focused[0])
			//      if (item.Focused)
			//          return n;
			//}
			return 0;
		...
		public static int LV_InsertCol(int column = 0, string options = null, string title = null)
		{
			...
			//column++;
			//_ = LV_ModifyCol(column, options, null);
			//return column;
		}
		...
		public static bool LV_Modify(int row = 0, string options = null, string[] fields = null)
		{
			...
			//}
			//foreach (var opt in Options.ParseOptions(options.ToLowerInvariant()))
			//{
			//  var on = opt[0] != '-';
			//  var mode = opt.Substring(!on || opt[0] == '+' ? 1 : 0);
			//  switch (mode)
			//  {
			//      case Core.Keyword_Checked: item.Checked = on; break;
			//      case Core.Keyword_Focus: item.Focused = on; break;
			//      case Core.Keyword_Select: item.Selected = on; break;
			//      case Core.Keyword_Vis: item.EnsureVisible(); break;
			//      default:
			//      {
			//          if (mode.StartsWith(Core.Keyword_Icon) && int.TryParse(mode.Substring(Core.Keyword_Icon.Length), out var n))
			//              item.ImageIndex = n;
			//      }
			//      break;
			//  }
		...
		public static bool LV_ModifyCol(int column = -1, string options = null, string title = null)
		{
			...
			//      case Core.Keyword_Auto: col.AutoResize(ColumnHeaderAutoResizeStyle.ColumnContent); break;
			//      case Core.Keyword_AutoHdr: col.AutoResize(ColumnHeaderAutoResizeStyle.HeaderSize); break;
			//      case Core.Keyword_IconRight: break;
			//      case Core.Keyword_Float: break;
			//      case Core.Keyword_Integer: break;
			//      case Core.Keyword_Text: break;
			//      case Core.Keyword_Center: col.TextAlign = HorizontalAlignment.Center; break;
			//      case Core.Keyword_Left: col.TextAlign = HorizontalAlignment.Left; break;
			//      case Core.Keyword_Right: col.TextAlign = HorizontalAlignment.Right; break;
			//      case Core.Keyword_Case: break;
			//      case Core.Keyword_CaseLocale: break;
			//      case Core.Keyword_Desc: break;
			//      case Core.Keyword_Locale: break;
			//      case Core.Keyword_NoSort: break;
			//      case Core.Keyword_Sort: break;
			//      case Core.Keyword_SortDesc: break;
			//      case Core.Keyword_Unicode: break;
			//      default:
			//          int n;
			//          if (mode.StartsWith(Core.Keyword_Icon, StringComparison.OrdinalIgnoreCase) && int.TryParse(mode.Substring(Core.Keyword_Icon.Length), out n))
			//              col.ImageIndex = n;
		...
		public static void LV_SetImageList(long id, int type = 0)
		{
			...
			//if (list == null)
			//  return;
			//if (ImageLists.IL_Get(id) is ImageList img)
			//{
			//  switch (type)
			//  {
