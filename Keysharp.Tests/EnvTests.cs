...
namespace Keysharp.Tests
{
	public partial class Scripting
	{
		...
		public void ClipWait()
		{
			...
			thread.Start();
			dt = DateTime.Now;
			Env.ClipWait(null, true);//Will wait indefinitely for any type.
			dt2 = DateTime.Now;
			...
			thread.Start();
			dt = DateTime.Now;
			Env.ClipWait();//Will wait indefinitely for only text or file paths.
			dt2 = DateTime.Now;
			ms = (dt2 - dt).TotalMilliseconds;
			tcs.Task.Wait();
			Assert.AreEqual(0, ErrorLevel);//Will have detected clipboard data, so ErrorLevel will be 0.
			...
			thread.Start();
			dt = DateTime.Now;
			Env.ClipWait(1);//Will wait for one second for only text or file paths.
			dt2 = DateTime.Now;
			ms = (dt2 - dt).TotalMilliseconds;
			tcs.Task.Wait();
			Assert.AreEqual(1, ErrorLevel);//Will have timed out, so ErrorLevel will be 1.
